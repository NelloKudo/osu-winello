...
# Proton-osu mirrors
PROTONLINK="https://github.com/whrvt/umubuilder/releases/download/proton-osu-9-1/proton-osu-9-1.tar.xz"
...
Icon=/home/$USER/.local/share/icons/osu-wine.png" | tee "$HOME/.local/share/applications/osuwinello-url-handler.desktop"
    ...
    if [ ! -d "$HOME/.local/share/wineprefixes/osu-wineprefix" ] ; then
        ...
        wget -O "$HOME/.winellotmp/osu-winello-prefix-umu.tar.xz" "https://gitlab.com/NelloKudo/osu-winello-prefix/-/raw/master/osu-winello-prefix-umu.tar.xz" && chk="$?" 
    
        # If download failed:
        if [ ! "$chk" = 0 ] ; then
            Info "wget failed; trying with --no-check-certificate.."
            wget --no-check-certificate -O "$HOME/.winellotmp/osu-winello-prefix-umu.tar.xz" "https://gitlab.com/NelloKudo/osu-winello-prefix/-/raw/master/osu-winello-prefix-umu.tar.xz" || failprefix="true"
        fi     
